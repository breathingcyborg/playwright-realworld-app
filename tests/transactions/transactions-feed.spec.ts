...
test.describe("pagination", () => {
  tabs.forEach((tab) => {
    test(`pagination works for ${tab.code} transactions`, async () => {
      ...
      // tab selected
      await expect(transactionsListPage.getTabButton(tab.code) as Locator).toHaveClass(
        /Mui-selected/
      );
...
test.describe("filters", () => {
  ...
  tabs.forEach((tab) => {
    test(`Shows ${tab.code} transactions within date range & lets clear date filter`, async () => {
      ...
      await expect(transactionsListPage.dateRangeButton).toHaveText(
        `Date: ${format(startDate, dateFormat)} - ${format(endDate, dateFormat)}`
      );
  ...
  tabs.forEach((tab) => {
    test(`${tab.code} displays items correctly`, async () => {
      ...
      for (let transaction of transactions) {
        ...
        await expect(transactionsListPage.getTransactionAmount(transaction.id)).toHaveText(
          expectedAmount
        );

        await expect(transactionsListPage.getTransactionSenderName(transaction.id)).toHaveText(
          transaction.senderName
        );

        await expect(transactionsListPage.getTransactionReceiverName(transaction.id)).toHaveText(
          transaction.receiverName
        );

        await expect(transactionsListPage.getTransactionLikesCount(transaction.id)).toHaveText(
          transaction.likes.length.toString()
        );

        await expect(transactionsListPage.getTransactionCommentsCount(transaction.id)).toHaveText(
          transaction.likes.length.toString()
        );
        ...
        await expect(transactionsListPage.getTransactionAction(transaction.id)).toHaveText(
          expectedAction!
        );
      }
    });
